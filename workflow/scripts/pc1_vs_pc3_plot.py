...
"""
Script to plot the PCA axes PC1 and PC3.
...
if os.stat(eigenvec).st_size > 0:
    ...
    if len(eig_vec.columns) > 4:
        pva = eig_val[0] / eig_val[0].sum() * 100 # calculate percentage variance explained
        eig_vec.rename(columns={0: 'FID', 1: 'IID', 2: 'PC1', 3: 'PC2', 4: 'PC3'}, inplace=True) 
        ...
        for k,d in grouped:
            ax.scatter(d['PC1'], d['PC3'], label=k, c=cividis.colors[n].reshape(1,-1))
            n+=1
        plt.legend(bbox_to_anchor=(1,1), loc="upper left")
        plt.xlabel('PC1' + ' (' + str(round(pva[0],1)) + ' %)')
        plt.ylabel('PC3' + ' (' + str(round(pva[2],1)) + ' %)')
else:
    fig,ax = plt.subplots()
    plt.xlabel('PC1')
    plt.ylabel('PC3')
