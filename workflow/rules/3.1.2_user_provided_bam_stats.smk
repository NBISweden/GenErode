...
if os.path.exists(config["historical_samples"]):
    if len(hist_user_bam_sm) > 0:
        userprov_bam_stats_outputs.extend("results/historical/mapping/" + REF_NAME + "/stats/bams_user_provided/multiqc/multiqc_report.html")

if os.path.exists(config["modern_samples"]):
    if len(mod_user_bam_sm) > 0:
        userprov_bam_stats_outputs.extend("results/modern/mapping/" + REF_NAME + "/stats/bams_user_provided/multiqc/multiqc_report.html")
...
rule historical_userprovided_bam_multiqc:
    ...
    input:
        expand("results/historical/mapping/" + REF_NAME + "/stats/bams_user_provided/{sample}.userprovided{extension}",
...
rule modern_userprovided_bam_multiqc:
    ...
    input:
        expand("results/modern/mapping/" + REF_NAME + "/stats/bams_user_provided/{sample}.userprovided{extension}",
